...
module.exports = (discounts) => {
  if (!discounts || !Array.isArray(discounts)) { throw new Error('Incorrect list of discounts') }
  ...
  const getValue = (articleId, price) => {
    if (!articleId || typeof articleId !== 'number' || articleId < 0) { throw new Error('Incorrect article id') }

    if (!price || typeof price !== 'number' || price < 0) { throw new Error('Incorrect price') }
