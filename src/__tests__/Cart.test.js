...
let mockArticles, mockDelivery, mockArticle, mockItems, cart

describe('Cart function', () => {
  beforeEach(() => {
    mockItems = [{ article_id: 1, quantity: 2 }, { article_id: 2, quantity: 3 }]
  ...
  describe('Checkout function', () => {
    ...
    it('should passes correct params to methods', () => {
      ...
      expect(mockArticles.findById.mock.calls[0][0]).toEqual(mockItems[0].article_id)
      expect(mockArticles.findById.mock.calls.length).toEqual(mockItems.length)

      expect(mockDiscount.getValue.mock.calls[0][0]).toEqual(mockItems[0].article_id)
    ...
    it('should return an erro when passed an incorrect list of items', () => {
      ...
      expect(() => cart.checkout(1, 'incorrect')).toThrow('Incorrect list of items')
    })
  })
})
